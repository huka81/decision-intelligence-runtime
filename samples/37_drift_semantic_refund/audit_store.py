"""
SQLite audit for Sample 37: decision_flows, context_snapshots, execution_log, decision_events.
"""
...
class AuditStore:
    """DFID-tagged flows, context snapshots, executions, and append-only decision events."""
    ...
    def _init_schema(self) -> None:
        self._conn.executescript(
            ...
            CREATE INDEX IF NOT EXISTS idx_context_snapshots_dfid ON context_snapshots(dfid);

            CREATE TABLE IF NOT EXISTS execution_log (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                dfid TEXT NOT NULL,
                refund_amount_eur REAL NOT NULL,
    ...
    def insert_context_snapshot(
        ...
        *,
        ...
        self._conn.execute(
            """
            INSERT INTO context_snapshots (dfid, snapshot_id, delay_hours, captured_at, detail_json)
            VALUES (?, ?, ?, ?, ?)
            """,
            (
                dfid,
    ...
    def insert_execution(
        ...
        *,
        ...
        cur = self._conn.execute(
            """
            INSERT INTO execution_log (dfid, refund_amount_eur, executed_at, detail_json)
            VALUES (?, ?, ?, ?)
            """,
            (
                dfid,
                float(refund_amount_eur),
                _utc_iso(),
                json.dumps(details or {}, sort_keys=True, default=str),
            ),
        )
    ...
    def rolling_refund_violation_rate(
        self,
        window: int,
        *,
        ...
        """
        Over the last `window` execution_log rows (by id DESC), join context_snapshots
        and compute fraction where delay_hours <= min_delay_hours_exclusive (semantic violation:
        refund issued but delay does not exceed policy threshold).
        ...
        row = self._conn.execute(
            """
            SELECT
              SUM(CASE WHEN cs.delay_hours <= ? THEN 1 ELSE 0 END) AS viol,
              COUNT(*) AS cnt
            FROM (
              SELECT el.id, el.dfid
              FROM execution_log el
              ORDER BY el.id DESC
              LIMIT ?
            ) w
            INNER JOIN execution_log el ON el.id = w.id
            INNER JOIN context_snapshots cs ON cs.dfid = el.dfid
            """,
            (min_delay_hours_exclusive, window),
        ).fetchone()
        if row is None or row["cnt"] is None or int(row["cnt"]) != window:
            return None
        viol = int(row["viol"] or 0)
        return viol / float(window)

    def list_executions_chronological(self) -> List[dict[str, Any]]:
        rows = self._conn.execute(
            """
            SELECT el.id, el.dfid, el.refund_amount_eur, el.executed_at, el.detail_json,
                   df.agent_id, df.status AS flow_status, df.input_ref,
                   cs.delay_hours, cs.snapshot_id
            FROM execution_log el
            ...
            out.append(
                {
                    "id": r["id"],
                    "dfid": r["dfid"],
                    "refund_amount_eur": r["refund_amount_eur"],
