...
<p>Section 3 shows a static SVG dual-panel view (verdict signal per scenario row and rolling
match against <code>scenarios.yaml</code> <code>expected</code>). Sections 4–5 reconstruct the
audit trail from canonical <code>decision_audit_events</code> only.</p>
"""
...
def _svg_dual_panel_chart(
    trace: List[Dict[str, Any]],
    expected_by_label: Dict[str, str],
) -> str:
    if not trace:
        return (
            '<figure aria-label="Scenario outcomes chart">'
            "<p class=\"muted\">No trace rows in this run window — chart omitted.</p></figure>"
        )
    n = len(trace)
    w = 720
    h = 320
    ...
    plot_w = w - 2 * pad
    plot_h = (h - 2 * pad) // 2 - 8
    xs = [pad + i * plot_w / max(n, 1) + plot_w / max(n, 1) / 2 for i in range(n)]
    bar_w = max(8.0, plot_w / max(n, 1) * 0.55)
    ...
    for i, item in enumerate(trace):
        ...
        v = _trace_verdict(ev)
        match = _expected_match(v, exp) if exp else False
        if v == "ACCEPT":
            fill = "#3fb950" if match else "#d29922"
        elif v == "REJECT":
            fill = "#3fb950" if match else "#f85149"
        else:
            fill = "#d29922"
        x = xs[i] - bar_w / 2
        y2 = pad + plot_h
        y1 = y2 - plot_h * (1.0 if v == "ACCEPT" else 0.45 if v == "REJECT" else 0.35)
        rects_a.append(
            f'<rect x="{x:.1f}" y="{y1:.1f}" width="{bar_w:.1f}" height="{y2 - y1:.1f}" '
            f'fill="{fill}" stroke="#30363d"/>'
        )

    mid_y = pad + plot_h + 20
    rolling_pts: List[Tuple[float, float]] = []
    ok_run = 0
    for i, item in enumerate(trace, start=1):
        ...
        exp = expected_by_label.get(label, "")
        v = _trace_verdict(ev)
        if exp and _expected_match(v, exp):
            ok_run += 1
        frac = ok_run / i if i else 0.0
        x = pad + (i - 0.5) * plot_w / max(n, 1)
        y = mid_y + plot_h * (1.0 - frac)
        rolling_pts.append((x, y))

    path_d = ""
    if rolling_pts:
        path_d = "M " + " L ".join(f"{px:.1f},{py:.1f}" for px, py in rolling_pts)
    ...
    return f"""
<figure aria-label="Per-scenario DIM verdict and rolling expected-match rate">
  <svg xmlns="http://www.w3.org/2000/svg" width="{w}" height="{h}" viewBox="0 0 {w} {h}">
    <text x="{pad}" y="18" fill="#8b949e" font-size="12">Panel A — scenario index (X) vs verdict bar height (Y)</text>
    <line x1="{pad}" y1="{pad}" x2="{w - pad}" y2="{pad}" stroke="#30363d"/>
    <line x1="{pad}" y1="{pad + plot_h}" x2="{w - pad}" y2="{pad + plot_h}" stroke="#30363d"/>
    {"".join(rects_a)}
    <text x="{pad}" y="{mid_y}" fill="#8b949e" font-size="12">
      Panel B — rolling match rate (Y) vs scenario index (X)
    </text>
    <line x1="{pad}" y1="{mid_y + plot_h}" x2="{w - pad}" y2="{mid_y + plot_h}" stroke="#30363d"/>
    {path_el}
  </svg>
  <figcaption class="muted">Green bars in Panel A mean DIM verdict matches declared
  <code>expected</code> for that scenario label; amber means ACCEPT but unexpected;
  red means mismatch. Shaded logic follows the same rule for REJECT.</figcaption>
</figure>
"""
...
def write_finops_langchain_html_report(
    ...
    *,
    ...
    chart_html = _svg_dual_panel_chart(trace, expected_by_label)
